...
const API_BASE = IS_GITHUB_PAGES ? "" : (window.location.port === "8001" ? "" : 'http://127.0.0.1:8001');
...
const getSamplesPrefix = () => {
  ...
  // For local files or local servers where samples are in the parent directory
  if (window.location.protocol === 'file:' || window.location.pathname.includes("/frontend/")) {
    return "../samples";
  }
  return "samples";
};
...
const DermAPI = {
  ...
  getSamplesUrl(url) {
    if (IS_GITHUB_PAGES || !this.isBackendOnline) {
      return url;
    }
    return `${API_BASE}${url}`;
  },
